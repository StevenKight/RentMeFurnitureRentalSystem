...
public class LoginDal
{
    ...
    public static bool CreateLogin(Login login, MySqlConnection? transaction = null)
    {
        ...
        try
        {
            connection.Execute(QueryStrings.CreateLogin,
                new { username = login.Username, password = PasswordHasher.HashPassword(login.Password) });
            return true;
        }
        catch (Exception ex)
        {
    ...
    public static bool ChangeLogin(string username, int id, string newPassword)
    {
        ...
        try
        {
            ...
            var result = connection.Execute(QueryStrings.ChangeLogin, parameters,
